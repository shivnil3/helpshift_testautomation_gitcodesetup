...
public class SubmitContactUsForm {
	 ...
	 * @param iteration
	 */
	public SubmitContactUsForm(String browserName, int iteration) {
		loop = iteration;
		browserToUse = browserName;
	}

	/**
	 * Main method for this class which has the test steps.
	 * 
	 */
	public void runTestcase() {
		...
		String emailTestString = "shivnil2@gmail.com";
		String issueDetailsTestString = "test issue";
		...
		try {
			for (loopCounter = 0; loopCounter < loop; loopCounter++) {
				...
				contactusFormObj.getContactNameTextBox().sendKeys(
						nameTestString);
				ResultsLogger.logInfo("Name Textbox value is set to "
						+ nameTestString);
				...
				contactusFormObj.getContactEmailTextBox().sendKeys(
						emailTestString);
				ResultsLogger.logInfo("Email Textbox value is set to "
						+ emailTestString);
